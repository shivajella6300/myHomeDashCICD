...
const StatUploadForm = () => 
  ...
  const validateForm = () => {
    const newErrors = {};
    const requiredFields = ['date', 'invoice','invoicedate', 'stationery_item_no', 'quantity','file'];
    ...
    if (formData.items.length > 1) 
    {
      setFormData(prev => ({
        ...
        items: prev.items.filter((_, i) => 
          ...
          //Here not equals data will come here 
         i !== index
  ...
  const handleSubmit = async(e) => {
    ...
    try
    {
     const responseData= await fetch('http://127.0.0.1:8000/api/stat-upload',
    {
      method:"POST",
      headers:
      {
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                   ...
                   {/* Inovice number */}
                    ...
                    <div className="flex items-center">
                      ...
                      <input
                        ...
                        className={`${getInputStyle('invoice')} h-7 text-xs px-2 w-full `} // same CSS kept
                      />
                    </div>
                ...
                <div className="flex items-center">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className={`${getInputStyle('invoicedate')} py-1 text-xs bg-transparent`} // ⬅️ reduced padding and text size
                  />
                </div>
               ...
               <div  className="mb-2 hidden"> 
                <div className="flex items-center">
                  ...
                  <input
                    ...
                    className={`${getInputStyle('file')} py-1 text-xs`} // ⬅️ reduced padding and text size
                  />
                </div>
                 {errors.file && attempted && (
                  <div className=" ml-1/3 pl-32 text-red-500 text-xs mt-0.5">{errors.file}</div>
                )}
              </div>

              <div className="mt-2 mx-8  ">
              <div className="flex items-center justify-between mb-2 ">
                <label className="text-indigo-800 font-bold text-xs">
                </label>
                <button
                  type="button"
